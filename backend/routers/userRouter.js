...
userRouter.get(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    // //for appointment
    const users1 = await Docprofile.insertMany(data.docprofiles);
    const doctor1 = users[0]._id;
    const doctor2 = users[1]._id;
    const sampleDocprofiles = data.doctorprofiles.map((docprofile, index) =>
      index < 3
        ? { ...doctorprofile, doctor: doctor1 }
        : { ...doctorprofile, doctor: doctor2 }
    );
    const docprofiles = await Doctorprofile.insertMany(sampleDocprofiles);
    res.send({ users1, docprofiles });
...
userRouter.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (user) {
      ...
      // Doctor
      if (user.isDoctor) {
        user.doctor.name = req.body.doctor.name;
        user.doctor.logo = req.body.doctor.logo;
        //user.doctor.description = req.body.doctor.description;
      }
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (
      ...
      res.send({
        ...
        isDoctor: signedinUser.isDoctor,
